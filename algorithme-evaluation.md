...
Le modèle utilisé est une régression logistique, à ce stade sans interactions. Elle est aujourd'hui entrainée sur de la donnée allant de janvier 2016 à novembre 2018 (pous les raisons évoquées plus haut) et produit une prédiction à Février 2020 qui peut etre interprétée comme la situation pré-crise Covid de l’établissement.

### Variables d'apprentissage

Le modèle est entrainé sur les variables d'apprentissage suivantes («en base» signifiant obtenues telles quelles, ne nécessitant pas de calcul supplémentaire) :

- `apart_heures_consommees_cumulees` (en base)
- `apart_heures_consommees` (en base)
- `ratio_dette` (en base)
- `avg_delta_dette_par_effectif` (calculée: évolution moyenne de la dette sociale sur effectif sur les 3 derniers mois)
- `paydex_group` (calculée: retard de paiement moyen de l'entreprise — groupes : [0, 15, 30, 60, 90+[)
- `paydex_yoy` (calculée: évolution sur 12 mois du retard de paiement moyen de l'entreprise)
...
- à un niveau plus agrégé, par groupe de variables de même «thématique». Parmi ces groupes de variables, on trouve :
  ...
  - le recours à l'activité partielle, et les retards de paiement aux fournisseurs.

_À noter que, en juin 2021, les méthodes d'explication de Signaux Faibles ne sont implémentées que pour un modèle de régression logistique._
...
Sur les fiches établissement de l'application Signaux Faibles, une liste de variables préoccupantes est fournie pour justifier un niveau de risque fort ou modéré. Il s'agit de variables qui, chacune individuellement, peuvent contribuer à un niveau de risque allant jusqu'à 30%. A noter qu'il s'agit d'une capacité de contribution théorique maximale. Dans la pratique, ces variables contribuent de concert à un niveau de risque fort, et leur contribution individuelle est moins élevée que cette valeur. Ceci est du au modèle employé.
...
Uniquement la partie «apprentissage supervisé» du modèle peut être évaluée proprement, dans la mesure où la crise n'a pas produit tous ses effets économiques et que nous ne disposons pas de visibilité sur les défaillances futures pour évaluer la performance des corrections apportées.
...
Les paragraphes suivants indiquent de quelle manière cette reproductibilité est assurée.
...
Voir [évaluation du modèle - juin2021](evaluation-modele/juin2021.md).
